...
# Retrieve the SSH key from AWS Systems Manager Parameter Store (No decryption needed)
echo "Retrieving SSH key from SSM..."
aws ssm get-parameter --name "$PARAMETER_NAME" --query "Parameter.Value" --output text > $SSH_KEY_PATH
